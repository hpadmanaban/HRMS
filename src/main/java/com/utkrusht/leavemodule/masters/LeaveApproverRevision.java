...
public class LeaveApproverRevision{
    ...
    public static List<LeaveApproverRevision> readAll(String employeeCode,String leaveCode) throws SQLException
    {
        ...
        try
        {
            ...
            while(rs.next())
            {
                ...
                leaveApproverRevision.setRevisionDate(new java.util.Date(rs.getDate("revision_date").getTime()));
                leaveApproverRevision.setLeaveApproversList(LeaveApprover.readAll(employeeCode, leaveCode,new java.util.Date(rs.getDate("revision_date").getTime())));
    ...
    public static Date getRelevantRevisionDate(String employeeCode,String leaveCode,Date leaveStartDate) throws SQLException
    {
        ...
        try
        {
            ...
            while(rs.next())
            {
                relevantDate = new java.util.Date(rs.getDate("revision_date").getTime());
                break;
            }
            ps.close();
            rs.close();
    ...
    public static List<LeaveApprover> getDistinctApprovers(Date revisionDate,String employeeCode) throws SQLException
    {
        ...
        try
        {
            ...
            for(int i =0; i < leavesList.size();i++)
            {
                ...
                while(rs.next())
                {
                    relevantRevisionDate = new java.util.Date(rs.getDate("revision_date").getTime());
                    break;
                }
